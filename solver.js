...
window.onload = function () {
    ...
    funcId("cross").addEventListener("click", function () {
        explanation.style.display = 'none';                     // Done to add opacity to the background after info-box is crossed out
...
var delChecker = 0;     // Variable kept to ensure smooth functioning of the delete option and making sure that delete itself dosent come on the board and makes sure to erase the numebers

function makeBoard () {

    clearOldBoard();    // Clears previous board and makes new one
    ...
    for(let i=0; i<81; i++){
        ...
        funcId("board").children[i].addEventListener("click", async function () {
            ...
            if(funcId("board").children[i].innerHTML != '' && numToAdd == 'del' && delChecker == 0) {
                ...
                if(numid%9 != 0) {board[Math.floor(numid/9)][(numid%9)-1] = 0;}    // Updating the value to 0 in our board list as the num is deleted
            ...
            if(checkDuplicates(board, parseInt(numToAdd), finalIndexes) == true && numToAdd != undefined && delChecker == 0) {
                ...
                else {board[Math.floor((numid-1)/9)][8] = parseInt(numToAdd);}  // Updates the value in the board list
            }
